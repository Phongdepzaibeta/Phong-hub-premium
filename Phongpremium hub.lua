...
local Window = Fluent:CreateWindow({
    Title = "phong hub premium version 1",
...
local listfastattack = {'Normal Attack','Fast Attack','Super Fast Attack'}
    ...
    _G.FastAttackphong_Mode = Value
	if _G.FastAttackphong_Mode == "Fast Attack" then
		_G.Fast_Delay = 0.12
	elseif _G.FastAttackphong_Mode == "Normal Attack" then
		_G.Fast_Delay = 0.2
	elseif _G.FastAttackphong_Mode == "Super Fast Attack" then
		_G.Fast_Delay = 0.05
	end
end)
    ...
    function getHighestPoint()
        ...
        for r, v in pairs(game:GetService("Workspace").Map.MysticIsland:GetDescendants()) do
            if v:IsA("MeshPart") then
                if v.MeshId == "rbxassetid://75474257824650" then
                    return v
                end
            end
        end
...
while wait() do
    pcall(function()
        if _G.FastAttackphong then
            repeat wait(_G.DelayAttackPlayer)
               AttackNoCoolDown()
            until not _G.FastAttackphong
        end
    end)
end
end)
...
----------------------------------------------------------------------------------------------------------------------------------------------
InterfaceManager:SetFolder("phongScriptHub")
SaveManager:SetFolder("phongScriptHub/specific-game")
Window:SelectTab(1)
Fluent:Notify({
    Title = "phong Hub",
    Content = hỡi thế gian ai là người giỏi võ xin hãy dạy tôi thứ võ mua tài.",
    Duration = 8
})
